...
class InstrumentManager:
    ...
    def open_instrument(self, gpib_address, name):
        try:
            instrument = self.rm.open_resource(f'GPIB0::{gpib_address}::INSTR')
            self.instruments[name] = instrument
            return instrument
        except pyvisa.Error as e:
            raise ConnectionError(f"Failed to connect to {name} at GPIB {gpib_address}: {str(e)}")
...
def run_measurement(duration=100000, interval=1):
    with measurement_session() as manager:
        yoko1 = VoltageController(manager.open_instrument(2, "YOKOgate1"), "YOKOgate1")
        yoko2 = VoltageController(manager.open_instrument(11, "YOKOgate2"), "YOKOgate2")
        current_meter = CurrentMeter(manager.open_instrument(22, "Current Meter"))
